...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="heading-font text-2xl font-extrabold tracking-tight text-slate-900 flex items-center gap-2">
                <i class="bx bx-trash text-rose-500"></i> Trashed Questions
            </h1>
            <p class="text-sm text-slate-500">
                Restore or permanently remove soft-deleted examination questions.
            </p>
        </div>

        <div>
            <a href="<?= route_to('admin.questions.index') ?>"
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-100/40">
        <?= view('admin/questions/_filter', [
            'searchPlaceholder' => 'Search trashed questions...',
            'exams'             => $exams ?? [],
        ]) ?>

        <?= view('admin/questions/_table', ['questions' => $questions, 'isTrash' => true]) ?>
