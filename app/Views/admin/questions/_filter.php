    ...
    <form action="<?= current_url() ?>" method="get" class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="relative flex-1 max-w-md">
            ...
            <input
                ...
                placeholder="<?= esc($searchPlaceholder ?? 'Search questions...') ?>"
        ...
        <div class="flex flex-wrap items-center gap-2.5">
            <?php if (!empty($exams)): ?>
                <div class="relative">
                    <?php $currentExam = request()->getGet('exam_id'); ?>
                    <select name="exam_id" onchange="this.form.submit()" class="appearance-none rounded-xl border border-slate-300/80 bg-white pl-3.5 pr-8 py-2 text-sm text-slate-700 transition outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10">
                        <option value="">All Exams</option>
                        <?php foreach ($exams as $exam): ?>
                            <option value="<?= $exam['id'] ?>" <?= (string) $currentExam === (string) $exam['id'] ? 'selected' : '' ?>>
                                <?= esc($exam['title']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            $hasExam   = !empty(request()->getGet('exam_id'));
            ?>
            <?php if ($hasSearch || $hasStatus || $hasExam): ?>
