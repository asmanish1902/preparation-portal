...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="heading-font text-2xl font-extrabold tracking-tight text-slate-900 flex items-center gap-2">
                <i class="bx bx-trash text-rose-500"></i> Trashed Exams
            </h1>
            <p class="text-sm text-slate-500">
                Restore or permanently remove soft-deleted exam configurations.
            </p>
        </div>

        <div>
            <a href="<?= route_to('admin.exams.index') ?>"
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-100/40">
        <?= view('admin/exams/_filter', [
            'searchPlaceholder' => 'Search trashed exams...',
            'categories'        => $categories ?? [],
            'subjects'          => $subjects ?? [],
        ]) ?>

        <?= view('admin/exams/_table', ['exams' => $exams, 'isTrash' => true]) ?>
