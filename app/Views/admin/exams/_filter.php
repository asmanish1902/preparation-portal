    ...
    <form action="<?= current_url() ?>" method="get" class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="relative flex-1 max-w-md">
            ...
            <input
                ...
                placeholder="<?= esc($searchPlaceholder ?? 'Search exams...') ?>"
        ...
        <div class="flex flex-wrap items-center gap-2.5">
            ...
            <?php if (!empty($categories)): ?>
                <div class="relative">
                    <?php $currentCat = request()->getGet('category_id'); ?>
                    <select name="category_id" onchange="this.form.submit()" class="appearance-none rounded-xl border border-slate-300/80 bg-white pl-3.5 pr-8 py-2 text-sm text-slate-700 transition outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10">
            ...
            <?php if (!empty($subjects)): ?>
                <div class="relative">
                    <?php $currentSub = request()->getGet('subject_id'); ?>
                    <select name="subject_id" onchange="this.form.submit()" class="appearance-none rounded-xl border border-slate-300/80 bg-white pl-3.5 pr-8 py-2 text-sm text-slate-700 transition outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10">
                        <option value="">All Subjects</option>
                        <?php foreach ($subjects as $sub): ?>
                            <option value="<?= $sub['id'] ?>" <?= (string) $currentSub === (string) $sub['id'] ? 'selected' : '' ?>>
                                <?= esc($sub['name']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            $hasCategory = !empty(request()->getGet('category_id'));
            $hasSubject  = !empty(request()->getGet('subject_id'));
            ?>
            <?php if ($hasSearch || $hasStatus || $hasCategory || $hasSubject): ?>
