...
<div class="space-y-6 max-w-full mx-auto">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-sm text-slate-500">
                Modify access taxonomies, descriptors, and sorting arrays for <span class="font-semibold text-slate-700"><?= esc($category['name']) ?></span>.
            </p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm shadow-slate-100/40">
        <?= view('admin/categories/_form', [
            'category' => $category,
            'action'   => route_to('admin.categories.update', $category['id']),
            'method'   => 'PUT',
        ]) ?>
    </div>
