...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">
                Manage, sort, and organize test subject segments across the system portal.
            </p>
        </div>

        <div class="flex items-center gap-3">
            <!-- Trash View Anchor Link -->
            <a href="<?= route_to('admin.categories.trash') ?>"
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-100/40">
        <!-- Render Filters Partial (Singular: _filter) -->
        <?= view('admin/categories/_filters') ?>

        <!-- Render Categories Data Table -->
        <?= view('admin/categories/_table', ['categories' => $categories, 'isTrash' => false]) ?>

        <!-- Pagination Footer -->
        <?php if (isset($pager) && !empty($categories)): ?>
