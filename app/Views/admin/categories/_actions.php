<div class="flex items-center justify-center gap-1.5">
    <?php if (isset($isTrash) && $isTrash): ?>
        <!-- Restore Button -->
        <form action="<?= route_to('admin.categories.restore', $category['id']) ?>" method="post" class="inline-block">
            <?= csrf_field() ?>
        ...
        <form action="<?= route_to('admin.categories.forceDelete', $category['id']) ?>" method="post" class="inline-block" onsubmit="return confirmForceDelete(event)">
            <?= csrf_field() ?>
    ...
    <?php else: ?>
        <!-- Edit Button -->
        <a href="<?= route_to('admin.categories.edit', $category['id']) ?>"
        ...
        <!-- Soft Delete Button -->
        <form action="<?= route_to('admin.categories.delete', $category['id']) ?>" method="post" class="inline-block" onsubmit="return confirmSoftDelete(event)">
            <?= csrf_field() ?>
            <input type="hidden" name="_method" value="DELETE">
            <button type="submit"
                class="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600"
